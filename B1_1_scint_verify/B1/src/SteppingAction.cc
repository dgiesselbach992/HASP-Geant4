...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    // Only care about steps inside the second scintillator
    if (volName == "Scintillator 2") {
		...
		if (it == scintMap.end()) {
			...
			runAction->ScintCount(particleName, track->GetParentID());
		}
    }
    // ------------------------------------------------------------
	// Scintillator Entrance tracking
	// ------------------------------------------------------------
	if (prePV && prePV->GetName() != "Scintillator 2" &&
		postPV && postPV->GetName() == "Scintillator 2" &&
		postPoint->GetStepStatus() == fGeomBoundary) {
		auto* track = step->GetTrack();
		std::string name = track->GetDefinition()->GetParticleName();
		fEventAction->GetRunAction()->EnterCount(name, track->GetParentID());
	}
	// ------------------------------------------------------------
	// Scintillator Exit tracking
	// ------------------------------------------------------------
	if (prePV && prePV->GetName() == "Scintillator 2" &&
		postPV && postPV->GetName() != "Scintillator 2" &&
		postPoint->GetStepStatus() == fGeomBoundary) {
		auto* track = step->GetTrack();
		std::string name = track->GetDefinition()->GetParticleName();
		fEventAction->GetRunAction()->ExitCount(name, track->GetParentID());
	}
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
}  // namespace B1
