...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // Shape 2
  //
  G4String shape2_name = "Scintillator";
  ...
  G4double shape2_z = 1 * cm;
  auto solidShape2 =
    new G4Box(shape2_name,  // its name
              0.5 * shape2_x, 0.5 * shape2_y,
              0.5 * shape2_z);  // its size
  ...
  // Shape 3
  //
  G4String shape3_name = "SiPM";
  G4Material* shape3_mat = nist->FindOrBuildMaterial("G4_Si");
  G4ThreeVector pos3 = G4ThreeVector(0, 0, 1 * cm);

  // Rectangluar shape?
  G4double shape3_x = 3 * cm;
  G4double shape3_y = 3 * cm;
  G4double shape3_z = 0.5 * cm;
  auto solidShape3 =
	new G4Box(shape3_name,  // its name
              0.5 * shape3_x, 0.5 * shape3_y,
              0.5 * shape3_z);  // its size

  auto logicShape3 = new G4LogicalVolume(solidShape3,  // its solid
                                         shape3_mat,  // its material
                                         shape3_name);  // its name
...
void DetectorConstruction::ConstructSDandField()
{
    ...
    auto sipmSD = new B1::SiPMSD("SiPMSD");
    sdManager->AddNewDetector(sipmSD);

    auto lvStore = G4LogicalVolumeStore::GetInstance();
    auto logicSiPM = lvStore->GetVolume("SiPM");
