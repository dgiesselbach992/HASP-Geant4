...
for i in $(seq 1 $NUM_RUNS); do
    ...
    # ------------------------------------------------------------
    # Extract blocks using robust patterns
    # ------------------------------------------------------------

    # ANY detector
    BLOCKALL=$(echo "$OUTPUT" | awk '/Particles that scintillated \(any detector\)/{flag=1; next} /^$/{flag=0} flag')

    # Scintillators 1 & 2
    BLOCK12=$(echo "$OUTPUT" | awk '/scintillated in Scintillators 1 & 2/{flag=1; next} /^$/{flag=0} flag')

    # Scintillators 1, 2 & 3
    BLOCK123=$(echo "$OUTPUT" | awk '/scintillated in Scintillators 1, 2 & 3/{flag=1; next} /^$/{flag=0} flag')

    # Tungsten exit (your volume name is Tungsten(Target))
    BLOCKTUNG=$(echo "$OUTPUT" | awk '/Tungsten\(Target\)/{flag=1; next} /^$/{flag=0} flag')
    ...
    while read -r line; do
        ...
        val=$(echo "$line" | cut -d: -f2)
        col=$(map_particle "$name")
        COUNTALL[$col]=$((COUNTALL[$col] + val))
    done <<< "$(echo "$BLOCKALL" | grep :)"
