...
class RunAction : public G4UserRunAction
{
  public:
	...
	// Proton-aware versions (name + parentID)
	void AddScintAll(const std::string& name, G4int parentID);
	void AddScint12(const std::string& name, G4int parentID);
	void AddScint123(const std::string& name, G4int parentID);
	void AddTungstenExit(const std::string& name, G4int parentID);
  ...
  private:
	...
	// --- New scintillation tallies ---
	std::map<std::string, int> fScintAllCounts;   // any scintillation
	std::map<std::string, int> fScint12Counts;    // scintillated in 1 & 2
	std::map<std::string, int> fScint123Counts;   // scintillated in 1, 2 & 3

	// --- New tungsten production tally ---
	std::map<std::string, int> fTungstenExitCounts;
