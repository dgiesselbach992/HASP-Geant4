...
std::string ClassifyParticle(const std::string& name, G4int parentID) {
    ...
    return name; // all other particles unchanged
}
...
void RunAction::AddScintAll(const std::string& name, G4int parentID) {
    std::string key = ClassifyParticle(name, parentID);
    fScintAllCounts[key]++;
}

void RunAction::AddScint12(const std::string& name, G4int parentID) {
    std::string key = ClassifyParticle(name, parentID);
    fScint12Counts[key]++;
}

void RunAction::AddScint123(const std::string& name, G4int parentID) {
    std::string key = ClassifyParticle(name, parentID);
    fScint123Counts[key]++;
}

void RunAction::AddTungstenExit(const std::string& name, G4int parentID) {
    std::string key = ClassifyParticle(name, parentID);
    fTungstenExitCounts[key]++;
}
...
void RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  if (!IsMaster()) {
	...
	if (IsMaster()) {
		...
		G4cout << "\n--- Particles that scintillated (any detector) ---\n";
		for (const auto& kv : fScintAllCounts)
			G4cout << kv.first << ": " << kv.second << G4endl;

		G4cout << "\n--- Particles that scintillated in Scintillators 1 & 2 ---\n";
		for (const auto& kv : fScint12Counts)
			G4cout << kv.first << ": " << kv.second << G4endl;

		G4cout << "\n--- Particles that scintillated in Scintillators 1, 2 & 3 ---\n";
		for (const auto& kv : fScint123Counts)
			G4cout << kv.first << ": " << kv.second << G4endl;

		G4cout << "\n================ Tungsten Exit Summary ================\n";
		for (const auto& kv : fTungstenExitCounts)
			G4cout << kv.first << ": " << kv.second << G4endl;
	...
	if (!IsMaster()) {
		...
		// ----------------------
		{
			std::ofstream ft("tungsten.csv", std::ios::app);
			...
			for (const auto& c : cols) {
				auto it = fTungstenExitCounts.find(c);
				int val = (it != fTungstenExitCounts.end()) ? it->second : 0;
				ft << "," << val;
			}
			ft << "\n";
			}
		...
		// ----------------------
		{
			std::ofstream fs("scint123.csv", std::ios::app);
			...
			fs << runID;
			for (const auto& c : cols) {
				auto it = fScint123Counts.find(c);
				int val = (it != fScint123Counts.end()) ? it->second : 0;
				fs << "," << val;
			}
			fs << "\n";
			}
