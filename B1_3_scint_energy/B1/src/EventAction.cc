...
void EventAction::EndOfEventAction(const G4Event* event)
{
    ...
    G4cerr << eventID << ","
           ...
           << d3 << "\n"; */
	// ------------------------------------------------------------
	// Classify scintillation per track
	// ------------------------------------------------------------
	for (const auto& entry : fScintMap) {
		const auto& info = entry.second;

		bool hit1 = info.detectors.count("Scintillator 1");
		bool hit2 = info.detectors.count("Scintillator 2");
		bool hit3 = info.detectors.count("Scintillator 3");

		const std::string& name = info.particleName;

		// Any scintillation already counted in SteppingAction

		// Scintillated in 1 & 2
		if (hit1 && hit2 && !hit3) {
			fRunAction->AddScint12(name, info.parentID);
		}

		// Scintillated in 1 & 2 & 3
		if (hit1 && hit2 && hit3) {
			fRunAction->AddScint123(name, info.parentID);
		}
}
}
