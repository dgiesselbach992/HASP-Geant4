...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // --- Custom Tungsten Material ---
  G4Material* W = nist->FindOrBuildMaterial("G4_W");

 // Extract composition
  G4int nElements = W->GetNumberOfElements();
  G4double newDensity = 17.6 * g/cm3;

  customW = new G4Material("CustomW", newDensity, nElements);

  for (G4int i = 0; i < nElements; i++) {
      customW->AddElement(
          const_cast<G4Element*>(W->GetElement(i)),
          W->GetFractionVector()[i]
      );
 }
  ...
  // G4Material* shape1_mat = customW;
  G4ThreeVector pos1 = G4ThreeVector(0, 0, -4.965 * cm);

  // Rectangluar shape
  G4double shape1_x = 3.81 * cm;
  G4double shape1_y = 3.81 * cm;
  G4double shape1_z = 11.43 * cm;
  auto solidShape1 =
  ...
  G4ThreeVector pos2 = G4ThreeVector(0, 0, 2.25 * cm);
  ...
  G4ThreeVector pos3 = G4ThreeVector(0, 0, 4.25 * cm);
  ...
  G4Material* shape5_mat = nist->FindOrBuildMaterial("G4_AIR");
  G4ThreeVector pos5 = G4ThreeVector(0, 0, 6.75 * cm);
  ...
  G4ThreeVector pos6 = G4ThreeVector(0, 0, 8.75 * cm);
  ...
  G4ThreeVector pos7 = G4ThreeVector(0, 0, 9.5 * cm);
  ...
  G4Material* shape8_mat = nist->FindOrBuildMaterial("G4_AIR");
  G4ThreeVector pos8 = G4ThreeVector(0, 0, 11.25 * cm);
  ...
  G4ThreeVector pos9 = G4ThreeVector(0, 0, 13.25 * cm);
  ...
  G4ThreeVector pos10 = G4ThreeVector(0, 0, 14 * cm);
